...
// ── Market Bias (SPY/QQQ vs prev close) ──────────────────────────────────────
...
async function updateMarketBias() {
  try {
    ...
    const [spy, qqq] = await Promise.allSettled([
      polygonGet(`/v2/aggs/ticker/SPY/range/1/day/${from}/${to}?adjusted=true&sort=desc&limit=3`),
      polygonGet(`/v2/aggs/ticker/QQQ/range/1/day/${from}/${to}?adjusted=true&sort=desc&limit=3`),
    ]);
    let score = 0;
    if (spy.status === "fulfilled" && spy.value?.results?.length >= 2) {
      spyData = spy.value.results;
      if (spyData[0].c > spyData[1].c) score++; else score--;
      if (spyData[0].c > (spyData[0].o + spyData[0].c) / 2) score++;
    }
    if (qqq.status === "fulfilled" && qqq.value?.results?.length >= 2) {
      qqqData = qqq.value.results;
      if (qqqData[0].c > qqqData[1].c) score++; else score--;
    }
    marketBias = score >= 2 ? "BULLISH" : score <= -2 ? "BEARISH" : "NEUTRAL/CHOPPY";
    log(`Market bias: ${marketBias} (score ${score})`);
  } catch(e) { log(`Bias update: ${e.message}`); }
}
...
// ══════════════════════════════════════════════════════════════════════════════
// ── GAP SCANNER (8:30am ET pre-market) ───────────────────────────────────────
// ══════════════════════════════════════════════════════════════════════════════
let gapSentToday = false;

async function sendGapScanner() {
  if (gapSentToday) return;
  gapSentToday = true;
  ...
  try {
    const GAP_TICKERS = [
      ...
      "NVDA","AMD","TSLA","META","AAPL","MSFT","AMZN","GOOGL","AVGO","PLTR","CRWD","DDOG",
      "NET","COIN","SQ","UBER","SHOP","NFLX","MRVL","ARM","SMCI","CLSK","CIFR","IREN",
    ];
    ...
    const results = await Promise.allSettled(
      GAP_TICKERS.map(t => polygonGet(`/v2/aggs/ticker/${t}/range/1/day/${from}/${to}?adjusted=true&sort=desc&limit=3`))
    );

    const gaps = [];
    results.forEach((r, i) => {
      if (r.status !== "fulfilled" || !r.value?.results?.length >= 2) return;
      const bars = r.value.results;
      const today = bars[0], prev = bars[1];
      if (!today || !prev || !prev.c) return;
      const gapPct = ((today.o - prev.c) / prev.c * 100);
      const volRatio = prev.v > 0 ? today.v / (prev.v / 390) : 0; // vs avg bar vol
      if (Math.abs(gapPct) >= 3) {
        gaps.push({
          ticker: GAP_TICKERS[i],
          gapPct: gapPct.toFixed(1),
          gapDir: gapPct > 0 ? "UP" : "DOWN",
          openPrice: today.o.toFixed(2),
          prevClose: prev.c.toFixed(2),
...
const SEED_TICKERS = [
  ...
  "MVIS","OCGN","APLD","BTCS","GFAI","ATOS","RAIL","SDIG","UAVS","CLSK",
];
...
async function fetchScalpMeta(ticker) {
  try {
    ...
    const [daily, ref] = await Promise.allSettled([
      polygonGet(`/v2/aggs/ticker/${ticker}/range/1/day/${from}/${to}?adjusted=true&sort=desc&limit=15`),
      polygonGet(`/v3/reference/tickers/${ticker}`),
    ]);
    if (daily.status === "fulfilled" && daily.value?.results?.length >= 3)
      scalpAvgVol[ticker] = daily.value.results.slice(0, 10).reduce((s, r) => s + r.v, 0) / Math.min(10, daily.value.results.length);
    if (ref.status === "fulfilled" && ref.value?.results?.share_class_shares_outstanding)
      scalpFloat[ticker] = ref.value.results.share_class_shares_outstanding;
  } catch(e) {}
}
...
async function buildScalpUniverse() {
  ...
  try {
    ...
    if (snap?.tickers) snap.tickers.forEach(t => {
  ...
  return list.filter(t => !scalpFloat[t] || scalpFloat[t] <= 20e6);
}
...
function detectScalpFlag(ticker) {
  ...
  for (let poleLen = 3; poleLen <= 8; poleLen++) {
    for (let i = n - 1; i >= poleLen + 2; i--) {
      ...
      const poleGain = (b[i-1].close - b[ps].open) / b[ps].open;
      ...
      const poleTop  = b[i-1].close, poleBtm = b[ps].open, poleH = poleTop - poleBtm;
      ...
      // Skip if too extended above VWAP
      ...
      return {
        ...
        spreadPct: ((b[n-1].high - b[n-1].low) / currentPrice * 100).toFixed(2),
        ...
        float: scalpFloat[ticker] ? `${(scalpFloat[ticker] / 1e6).toFixed(1)}M` : "Unknown",
...
function connectScalpWS(tickers) {
  ...
  scalpWS.on("error", e => log(`Scalp WS: ${e.message}`));
}
...
function disconnectScalp() {
  ...
  log("Scalp stopped");
  sendTelegram(`🔴 <b>Bull Flag Scalper OFFLINE</b>\nBack tomorrow 5am ET · ${biasEmoji()} Final bias: ${marketBias}`);
}

async function checkScalpSchedule() {
  const et = getET(), h = et.getHours();
  if (isWeekday() && h >= 5 && h < 10) {
    if (!scalpActive) {
...
let revWS = null, revBars5m = {}, revLevels = {}, revAvgBarVol = {};
let revAlerted = new Set(), revActive = false, revReconnect = null;
...
async function fetchRevLevels(ticker) {
  try {
    ...
    const [daily, hourly] = await Promise.allSettled([
      polygonGet(`/v2/aggs/ticker/${ticker}/range/1/day/${from}/${to}?adjusted=true&sort=desc&limit=5`),
      polygonGet(`/v2/aggs/ticker/${ticker}/range/1/hour/${from}/${to}?adjusted=true&sort=asc&limit=120`),
    ]);
    const levels = { prevHigh: null, prevLow: null, prevClose: null, srLevels: [] };
    if (daily.status === "fulfilled" && daily.value?.results?.length >= 2) {
      const p = daily.value.results[1];
      levels.prevHigh = p.h; levels.prevLow = p.l; levels.prevClose = p.c;
      revAvgBarVol[ticker] = p.v / 78;
    }
    if (hourly.status === "fulfilled" && hourly.value?.results?.length) {
      const hrs = hourly.value.results;
      const raw = [];
      for (let i = 2; i < hrs.length - 2; i++) {
        if (hrs[i].h > hrs[i-1].h && hrs[i].h > hrs[i+1].h) raw.push({ price: hrs[i].h, type: "resistance" });
        if (hrs[i].l < hrs[i-1].l && hrs[i].l < hrs[i+1].l) raw.push({ price: hrs[i].l, type: "support" });
      }
      const deduped = [];
      raw.sort((a, b) => a.price - b.price).forEach(l => { if (!deduped.find(d => Math.abs(d.price - l.price) / l.price < 0.005)) deduped.push(l); });
      levels.srLevels = deduped;
    }
...
function detectReversal(ticker) {
  ...
  for (let ei = n-1; ei >= n-3; ei--) {
    ...
    for (let poleLen = 4; poleLen <= 5; poleLen++) {
      ...
      levels.srLevels.forEach(l => { const pct = Math.abs(exhaustPrice - l.price) / exhaustPrice; if (pct <= 0.01) candidates.push({ price: l.price, type: l.type === "resistance" ? "1hr Resistance" : "1hr Support", pct }); });
      ...
      return { ticker, direction, emoji, currentPrice: exhaustBar.close.toFixed(2), poleLen, poleMove, marubozuCount, candleType, exhaustVol: Math.round(exhaustBar.vol), volSpike: avgVol > 0 ? (exhaustBar.vol / avgVol).toFixed(1) : "N/A", rsi, rsiStars, srLevel, prevHigh: levels.prevHigh?.toFixed(2), prevLow: levels.prevLow?.toFixed(2), prevClose: levels.prevClose?.toFixed(2), srLevels: levels.srLevels };
    }
  }
  return null;
}
...
function connectRevWS(tickers) {
  ...
  revWS.on("error", e => log(`Rev WS: ${e.message}`));
}
...
async function checkRevSchedule() {
  ...
  if (isWeekday() && etMins >= 570 && etMins < 630) {
    if (!revActive) {
...
function processOrbBar(ticker, bar) {
  ...
  if (etMins >= 580 && etMins < 615) {
    ...
    if (avgVol > 0 && bar.vol < avgVol * 2) return;
...
<i>Range locks 9:40 · Breakout window 9:40–10:15
Elite 🟢 and Good 🟡 setups only</i>`);
...
function connectOrbWS(tickers) {
  ...
  orbWS.on("error", e => log(`ORB WS: ${e.message}`));
}
...
// ══════════════════════════════════════════════════════════════════════════════
// ── DAILY RECAP + WIND-DOWN + SCHEDULER ──────────────────────────────────────
// ══════════════════════════════════════════════════════════════════════════════
let recapSentToday = false;
let windDownSentToday = false;
let gapScanScheduled = false;
let biasSentToday = false;
...
📐 ORB pre-market brief coming at 9:15am
🎯 Shift focus to opening range setups

${biasEmoji()} Market bias heading into open: <b>${marketBias}</b>
${marketBias === "BULLISH" ? "→ Favor long ORB setups" : marketBias === "BEARISH" ? "→ Favor short/breakdown setups" : "→ Wait for clear direction at open"}`);
  ...
  if (etMins >= 990 && etMins < 992 && !recapSentToday) {
    ...
    const orbAlertCount = todayAlerts.filter(a => a.type === "orb").length;
    const revAlertCount = todayAlerts.filter(a => a.type === "reversal").length;
...
⚡ Scalp alerts:    ${scalpAlerts}
📐 ORB alerts:      ${orbAlertCount}
🔄 Reversal alerts: ${revAlertCount}
📊 Total:           ${todayAlerts.length}

🎯 Traded: ${traded}  ·  Wins: ${wins}  ·  Losses: ${losses}
📈 Win Rate: ${wr}
⭐ High conviction (4-5★): ${highConv} alerts

<i>Review your trades in WickED dashboard
ORB brief tomorrow at 9:15am ET</i>`);
  ...
  if (etMins === 0) {
    ...
    windDownSentToday = false;
    gapScanScheduled = false;
    biasSentToday = false;
    orbBriefSent = false;
  }
}
...
// Update market bias every 30 min during trading hours
setInterval(async () => {
  const etMins = getETMins();
  if (isWeekday() && etMins >= 480 && etMins < 960) {
    await updateMarketBias();
  }
}, 30 * 60 * 1000);
